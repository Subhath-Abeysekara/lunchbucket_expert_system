...
def get_suitability(id1 , id2 ,id3, df , vege_menu , stew_menu , meat_menu):
    meat_suitability = []
    stew_suitability = []
    vegi_suitability = []
    for i in meat_menu:
        df2 = df.loc[df['number'] == i['number']]
        suitability = (int(df2[id1]) + int(df2[id2]) + int(df2[id3])) / 3
...
def threeSuitability_dev_lunch(id1,id2,id3):
    df = csv_dev
    item = collection_name_dev.find_one({'_id': ObjectId(TODAY_MENU_ID_DEV)})
    response = {
        "state": True,
        "data": get_suitability(id1=id1, id2=id2, id3=id3, df=df, meat_menu=item['meat_lunch'],stew_menu=item['stew_lunch'],vege_menu=item['vege_lunch'])
    }
    return response

def threeSuitability_dev_dinner(id1,id2,id3):
    df = csv_dev
    item = collection_name_dev.find_one({'_id': ObjectId(TODAY_MENU_ID_DEV)})
    response = {
        "state": True,
        "data": get_suitability(id1=id1, id2=id2, id3=id3, df=df, meat_menu=item['meat_dinner'],stew_menu=item['stew_dinner'],vege_menu=item['vege_dinner'])
    }
    return response

def threeSuitability_prod_lunch(id1,id2,id3):
    df = csv_prod
...
def threeSuitability_prod_dinner(id1,id2,id3):
    df = csv_prod
    item = collection_name_prod.find_one({'_id': ObjectId(TODAY_MENU_ID_PROD)})
    response = {
        "state": True,
